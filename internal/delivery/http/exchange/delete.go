...
func (h *ExchangeHandler) DeleteExchangeRequest(c *gin.Context) {
	...
	if err != nil {
		log.Printf("Failed To Get Exchange Request By ID to delete %v\n", fetchedExchangeRequest)
	...
	err = h.exchangeUsecase.DeleteExchangeRequest(fetchedExchangeRequest, loggedInUserID)
	if err != nil {
		log.Printf("Failed To Delete Exchange Request %v\n", err)
		c.JSON(response.Status(err), gin.H{
			"error": err,
		})
		return
