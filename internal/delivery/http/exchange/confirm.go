...
func (h *ExchangeHandler) ConfirmExchangeRequest(c *gin.Context) {
	...
	if err != nil {
		log.Printf("Failed To Get Exchange Request By ID to confirm %v\n", fetchedExchangeRequest)
	...
	err = h.exchangeUsecase.ConfirmExchange(fetchedExchangeRequest, loggedInUserID)
	if err != nil {
		log.Printf("Failed To Confirm Exchange Request %v\n", err)
		c.JSON(response.Status(err), gin.H{
			"error": err,
		})
		return
