...
func (h *ExchangeHandler) DeclineExchangeRequest(c *gin.Context) {
	...
	if err != nil {
		log.Printf("Failed To Get Exchange Request By ID to decline %v\n", fetchedExchangeRequest)
	...
	err = h.exchangeUsecase.DeclineExchange(fetchedExchangeRequest, loggedInUserID)
	if err != nil {
		log.Printf("Failed To Decline Exchange Request %v\n", err)
		c.JSON(response.Status(err), gin.H{
			"error": err,
		})
		return
