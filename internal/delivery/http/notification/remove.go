...
func (h *NotificationHandler) RemoveNotification(c *gin.Context) {
	...
	err = h.notificationService.RemoveNotification(fetchedNotification, loggedInUserID)
	if err != nil {
		log.Printf("Failed To Remove Notification %v\n", err)
		c.JSON(response.Status(err), gin.H{
			"error": err,
		})
		return
